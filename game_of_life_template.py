...
class Board(object):
    ...
    def __init__(self, win, width, height):
        ...
        for x in range(0,self.width):
            for y in range(0,self.height):
                self.block_list[(x,y)] = Block(Point(x,y),'black')
    ...
    def seed(self, block_coords):
        ...
        '''
        for coor in block_coords:
            blocklive = self.block_list[coor]
            blocklive.set_live(self.canvas)
    ...
    def get_block_neighbors(self, block):
        ...
        '''
        neighborblock = [] #final list
        coorslist = []
        coorslist.append((block.x+1,block.y+1))
        coorslist.append((block.x,block.y+1))
        coorslist.append((block.x-1,block.y+1))
        coorslist.append((block.x+1,block.y))
        coorslist.append((block.x-1,block.y-1))
        coorslist.append((block.x-1,block.y))
        coorslist.append((block.x+1,block.y-1))
        coorslist.append((block.x,block.y-1))
        ...
        for coor in coolist:         
            if coor[0] < 0 or coor[0] >= self.width or coor[1] < 0 or coor[1] >= self.height:
                coorslist.remove(coor)
                
        for coor in coorslist:
            neighborblock.append(self.block_list[coor])
