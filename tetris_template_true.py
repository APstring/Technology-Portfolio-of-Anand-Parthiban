...
class Block(Rectangle):
    ...
    def can_move(self, board, dx, dy):
        ...
        '''
        if Board.can_move(board,self.x + dx, self.y + dy):
            return True
        else:
            return False
...
class Shape():
    ...
    def can_move(self, board, dx, dy):
        ...
        for block in self.blocks:
            if block.can_move(board, dx, dy):
                checklist.append('y')
        if len(checklist) == len(self.blocks):
            return True
        else:
            return False
    ...
    def can_rotate(self, board):
        ...
        '''
        blockfact = []
        dire = self.get_rotation_dir()
        center = self.center_block
        for block in self.blocks:  
            #block.x = center.x - dire*center.y + dire*block.y
            #block.y = center.y + dire*center.x - dire*block.x
            new_block = Block(Point(center.x - dire*center.y + dire*block.y,center.y + dire*center.x - dire*block.x), block.color)
            if new_block.can_move(board,0,0):
                blockfact.append('y')
        if len(blockfact) == 4:
            return True
        else:
    ...
    def rotate(self, board):
        ...
        for block in self.blocks:
            if not block == center:
                block.move((center.x - dire*center.y + dire*block.y)- block.x ,(center.y + dire*center.x - dire*block.x)-block.y)
...
class Board():
    ...
    def move_down_rows(self, y_start):
        ...
        dicvals = []
        for block in self.grid.values():
            dicvals.append(block)
        for block in self.grid.keys():
            dickeys.append(block)
        for block in dickeys:
            if block[0] in range(0,11):
                if block[1] in range(0,y_start):
                    self.grid[(block[0], block[1]+1)] = dicvals[dickeys.index(block)]
                    block_new = dicvals[dickeys.index(block)]
                    block_new.move(0,1)
                    del self.grid[(block[0], block[1])]
    ...
    def game_over(self):
        ...
        '''
        Face = Rectangle(Point(0,self.height*45/2), Point(self.width*30, (self.height/4)*30))
        Face.setFill('blue')
        text = Text(Point(self.width*30/2, self.height*30/2),'Game Over!!!')
        text.setSize(20)
        text.setTextColor('green1')
...
class Tetris():
    ...
    def create_new_shape(self):
        ...
        new = randint(0, len(self.SHAPES)-1)
        shape = self.SHAPES[new](Point(int(self.BOARD_WIDTH/2),0))
        return shape
    ...
    def do_move(self, direction):
        ...
        '''
        dire = self.DIRECTION[direction]
        if self.current_shape.can_move(self.board, dire[0], dire[1]):
            self.current_shape.move(dire[0], dire[1])
            return True
        elif direction == 'Down':
            if not self.current_shape.can_move(self.board, dire[0], dire[1]):
                blooklist = []
                blook = self.current_shape.get_blocks()
                for block in blook:
                    blooklist.append(block.y)
                if not min(blooklist) == 0: 
    ...
    def key_pressed(self, event):
        ...
        if key == 'space':
            if not self.do_move('Down'):
                return  
            elif self.do_move('Down'):
                self.do_move('Down')
                self.win.after(0, self.key_pressed, event)
        elif key == 'Up':
            self.do_rotate()
        else:
            self.do_move(key)
